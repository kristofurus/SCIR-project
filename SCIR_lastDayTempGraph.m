...
[temp, time] = thingSpeakRead(sensorChannelID, 'Fields', temperatureFieldID, 'NumMinutes', NUM_OF_HOURS*60, 'ReadKey', sensorChannelReadKey);
...
median_temp = zeros(size(dates_unique, 1), NUM_OF_SENSORS);
neg_err = zeros(size(dates_unique, 1), NUM_OF_SENSORS);

for i = 1:size(dates_unique, 1)
    
    selected_temperatures = temp(dates == dates_unique(i), :);

    for j = 1:NUM_OF_SENSORS
        
        sensor = selected_temperatures(~isnan(selected_temperatures(:, j)), j);
       
        % get Q1, Q2, Q3
        q1 = prctile(sensor, 25);
        median_temp(i, j) = median(sensor);
        ...
        if ~isempty(sensor)
            ...
            % get IRQ
            IRQ = abs(q3-q1);
            pos_err(i, j) = abs(median_temp(i, j) - max(sensor(sensor <= q3+3/2*IRQ)));
            neg_err(i, j) = abs(median_temp(i, j) - min(sensor(sensor >= q1-3/2*IRQ)));
...
% plot data
stairs(dates_int, internet_temp , "--", "color", "#000000", "linewidth", 2);  
hold on
errorbar(repmat(dates_unique, [1, NUM_OF_SENSORS]), median_temp, neg_err, pos_err, '-o', "linewidth", 1.5, "MarkerSize",8, "capsize", 10);
hold off
grid minor
xlabel("date");
xticks(dates_unique(1:3:NUM_OF_HOURS));
ylabel("temperature [\circC]");
legend(["Internet", "SHT31", "BMP280", "DHT11", "DHT22", "DS18B20"], "Location", "eastoutside")
